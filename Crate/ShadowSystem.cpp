...
ShadowSystem::ShadowSystem()
    ...
    , mLightingCB(nullptr)
{
    mCascadeSplitsViewZ[0] = mCascadeSplitsViewZ[1] = mCascadeSplitsViewZ[2] = mCascadeSplitsViewZ[3] = 0.0f;
...
void ShadowSystem::TransitionShadowMap(ID3D12GraphicsCommandList* cmdList, ShadowMapState newState)
{
    ...
    if (mShadowMapState == ShadowMapState::Common && newState == ShadowMapState::DepthWrite)
        barrier.Transition.StateBefore = D3D12_RESOURCE_STATE_COMMON;
    else if (mShadowMapState == ShadowMapState::DepthWrite && newState == ShadowMapState::ShaderResource)
        barrier.Transition.StateBefore = D3D12_RESOURCE_STATE_DEPTH_WRITE;
    else if (mShadowMapState == ShadowMapState::ShaderResource && newState == ShadowMapState::DepthWrite)
        barrier.Transition.StateBefore = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
    else
        return;

    if (newState == ShadowMapState::DepthWrite)
        barrier.Transition.StateAfter = D3D12_RESOURCE_STATE_DEPTH_WRITE;
    else if (newState == ShadowMapState::ShaderResource)
